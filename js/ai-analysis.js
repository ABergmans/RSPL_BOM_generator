...
// FIX v2: generateSamplePartsData() removed — no fake part numbers ever generated
...
async function analyzeWithCohere(text, brand, model) {
    ...
    // ── Beperk tekst tot 6000 tekens om binnen token-limieten te blijven ─────────
    // Gebruik de eerste 6000 én de laatste 2000 tekens zodat kop EN staart van de
    // parts list meegenomen worden.
...
1. The "partNumber" field must contain the EXACT part number as it appears in the source text.
   Do NOT generate, invent or modify part numbers. Copy them character-for-character.
2. If no part number is visible for an item, leave "partNumber" as an empty string "".
3. Do NOT use placeholder codes like "HTR-0001", "PCB-0002" etc. — only real numbers from the text.

${analysisText}

For each part found in the text, extract:
...
- type        (Pr=Preventive, Cr=Corrective/Critical, Con=Consumable — infer from context)
...
Return ONLY valid JSON in this exact format — no markdown, no explanation, no code fences:
{
    ...
    try {
        const response = await fetch(COHERE_API_URL, {
            ...
            body: JSON.stringify({
                model: 'command-r',     // Upgegraded van 'command' naar 'command-r' voor betere JSON output
                prompt: prompt,
                max_tokens: 4000,       // Verhoogd van 2000 → 4000 voor grotere lijsten
                temperature: 0.1,       // Verlaagd van 0.3 → 0.1 voor meer deterministische output
                k: 0,
                stop_sequences: [],
        ...
        let parsedData;
        try {
            // Probeer JSON te extraheren uit de respons (met of zonder code fences)
            ...
            // ── KRITIEKE FIX: GEEN nep-data terugsturen ─────────────────────────
            // Gooi een duidelijke fout zodat de gebruiker weet wat er mis is.
            throw new Error(
        ...
        // ── Extra veiligheidscheck: verwijder onderdelen met duidelijk nep-nummers ──
        // Patronen zoals "HTR-0001", "PCB-0002" wijzen op AI-hallucinatie
        ...
        const cleanedParts = parts.map(part => {
            if (part.partNumber && fakePattern.test(part.partNumber)) {
                ...
                part.partNumber = ''; // Leeg maken zodat het zichtbaar ontbreekt
            }
            return part;
        });
        ...
        throw error;    // Gooi de fout door — GEEN nep-data als fallback
    }
}
